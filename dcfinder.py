...
class DCFinder():
    ...
    def find(self, gallery_id, keyword, search_type, search_pos=0, search_depth=-1):
        """
        ...
        for pos_idx in range(search_depth):
            articles = self.crawl_search(board_url, keyword, search_pos - (pos_idx * 10000), search_type)
            for article in articles:
                print(article)

    def crawl_search(self, board_url, keyword, search_pos, s_type):
        if search_pos < 0:
            search_pos = 0
        ...
        article_list = self.get_articles(parser)

        rest_lists = []
        # get rest of page
        for page_idx in range(2, max(page_len + 1, 2)):
            rest_lists.extend(self.get_page(board_url + search_query.format(page_num=page_idx, search_pos=search_pos, s_type=s_type, s_keyword=keyword)))

        article_list.extend(rest_lists)
        return article_list

    def get_page(self, url):
        resp = requests.get(url)
        parser = BeautifulSoup(resp.text, "html.parser")
        return self.get_articles(parser)
...
def main():
    dcfinder = DCFinder()
    dcfinder.find(갤러리, 키워드, 검색타입, search_depth=깊이)
